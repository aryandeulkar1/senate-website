...
const PrismaticBurst = ({
  ...
  // Update mouse target when mousePosition prop changes
  useEffect(() => {
    if (mousePosition) {
      mouseTargetRef.current = [mousePosition.x, mousePosition.y];
    }
  }, [mousePosition]);
  ...
  useEffect(() => {
    ...
    // Only add pointer listener if no external mousePosition prop
    let onPointer = null;
    if (!mousePosition) {
      onPointer = e => {
    ...
    return () => {
      ...
      if (gl && gradientTex?.texture) {
        gl.deleteTexture(gradientTex.texture);
      }
    };
  }, []);
